...
static void* led_red = NULL;
static void* led_green = NULL;
...
static long int my_ioctl(struct file*, unsigned int cmd, unsigned long arg)
{
	...
	case WR_R_DISPLAY:
		...
		break;
	case WR_RED_LEDS:
		write_pointer = led_red;
		write_name_index = 2 + 3;
		printk("my_driver: updated write pointer to %s\n", perf_names[write_name_index]);
		break;
	case WR_GREEN_LEDS:
		write_pointer = led_green;
...
static int my_pci_probe(struct pci_dev *dev, const struct pci_device_id *id)
{
	...
	led_red  = ioremap(resource + 0xC040, 0x20);
	led_green = ioremap(resource + 0xC060, 0x20);
